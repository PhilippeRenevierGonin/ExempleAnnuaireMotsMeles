...
constructor() {
    ...
    /*
    Pour les ligatures, adaptation d'un code : 
    ...
    */
    ...
    removeDiacritics (str) {
            return str.normalize("NFD").replace(/[\u0300-\u036f]/g, "").replace(/[^\u0000-\u007E]/g, a => this.diacriticsMap[a] || a);
        }    
...
reçoitMessage(message, piecejointe) {
    ...
    if (message == MESSAGE.MOT_TAPE) {
        ...
        if (piecejointe && piecejointe.liste) {
            piecejointe.liste.forEach(m => this.listes.push(new Mot(this.removeDiacritics(m))) );
        }
        else {
                // const listeInit = ["courageusement","agit","acquisitions","Paulhan","pigeonner","pèse","vital","cris","parité","donne","ri","boy","vive","pas","tes","sot","aux","but","ou","pro","sa","ais","al","ma","mou","né","ce","en","suc","si","elle","ira","base","lie","dû","feu","on","hocher","vus","victimes","uni","toutefois","eux","médicament","fur","automatiquement","téléspectateurs"];
            const listeInit = ["pneus","extorqueur","neveu","kurt","crampe","emetteurs","hourra","yeux","page","abrupt","juif","kanak","cob","paresser","vos","sa","tu","il","ben","je","ce","nom","répandue","predominance","vas","surveillants","chaos"];

          listeInit.forEach(m => this.listes.push(new Mot(this.removeDiacritics(m))) );
        }
...
reçoitMessage(message, piecejointe) {
    ...
    if (message == MESSAGE.CLEAR) {
        ...
        li.innerHTML = "<span style=''>"+piecejointe+"</span>";
...
reçoitMessageDuParent(message, piecejointe) {
    if (message == MESSAGE.MOT_TAPE) {
        ...
        this.pres.reçoitMessage(MESSAGE.CLEAR);
        this.abs.reçoitMessage(MESSAGE.INIT, piecejointe);
        this.abs.reçoitMessage(MESSAGE.LISTE);
    }
